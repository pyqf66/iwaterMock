...
(function($) {
	function _1(_2) {
		...
		(function				() {
					...
					onClick : function(_b) {
						if (_4.multiple) {
							$(this).tree(_b.checked ? "uncheck" : "check",
									_b.target);
						} else {
							$(_2).combo("hidePanel");
						}
	...
	function _e(_f) {
		...
		$(_f).combo("setText", ss.join(_11.separator)).combo("setValues",
				_11.multiple ? vv : (vv.length ? vv : [""]));
	};
	function _15(_16, _17) {
		...
		_1a.find("span.tree-checkbox").addClass("tree-checkbox0")
		...
		$.map(vv, function(v) {
					...
					if (_1f) {
						_1a.tree("check", _1f.target)
								.tree("select", _1f.target);
						ss.push(_1f.text);
					} else {
						ss.push(v);
					}
		...
		_1b.onCheck = _1c;
		_1b.onSelect = _1d;
		$(_16).combo("setText", ss.join(_19.separator)).combo("setValues",
				_19.multiple ? vv : (vv.length ? vv : [""]));
	};
	...
	$.fn.combotree.methods = {
		...
		clear : function(jq) {
			return jq.each(function() {
						...
						_2f.find("div.tree-node-selected")
	...
	$.fn.combotree.parseOptions = function(_31) {
		return $.extend({}, $.fn.combo.parseOptions(_31), $.fn.tree
						.parseOptions(_31));
	};
