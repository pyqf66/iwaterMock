...
(function($) {
	...
	function ajaxSubmit(target, options) {
		...
		frame.attr('src', window.ActiveXObject
						? 'javascript:false'
						: 'about:blank');
		frame.css({
					position : 'absolute',
		...
		function submit(param) {
			...
			try {
				for (var n in param) {
					var field = $('<input type="hidden" name="' + n + '">')
							.val(param[n]).appendTo(form);
	...
	function load(target, data) {
		...
		function _checkField(name, val) {
			...
			cc = $(target).find('input[name="' + name
		...
		function _loadBox(name, val) {
			var field = $(target).find('[textboxName="' + name
					+ '"],[sliderName="' + name + '"]');
			if (field.length) {
	...
	function setValidation(target, novalidate) {
		...
		$(target).find('.validatebox-text:not(:disabled)')
				.validatebox(novalidate
						? 'disableValidation'
						: 'enableValidation');
	}
