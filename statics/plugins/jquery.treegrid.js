...
(function($) {
	...
	function _27(_28, _29) {
		...
		var _2b = $(_28).datagrid("getColumnFields", true).length
				+ (_2a.rownumbers ? 1 : 0);
		...
		function _2d(tr, _2e) {
			$("<tr class=\"treegrid-tr-tree\">"
					+ "<td style=\"border:0px\" colspan=\"" + _2e + "\">"
	...
	function _2f(_30, _31, _32, _33) {
		...
		if (_36) {
			...
			var cc1 = _38.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			var cc2 = _39.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			if (!_33) {
				_36.children = [];
			}
	...
	function _85(_86, _87) {
		...
		var _89 = _4c(_86, ref);
		_7f(_86, {
					parent : (_89 ? _89[_88.idField] : null),
					data : [_87.data]
				});
	...
	function _91(_92, _93) {
		...
		_1f(_92);
		_94.total -= 1;
		$(_92).datagrid("getPager").pagination("refresh", {
					total : _94.total
				});
	...
	function _95(_96) {
		...
		function _9a(_9c) {
			...
			if (_9c.length) {
				...
				_9f.find(".tree-join").removeClass("tree-join")
	...
	$.fn.treegrid.methods = {
		...
		reloadFooter : function(jq, _b1) {
			return jq.each(function() {
						...
						if (_b2.showFooter) {
							_b2.view.renderFooter.call(_b2.view, this,
									dc.footer1, true);
							_b2.view.renderFooter.call(_b2.view, this,
									dc.footer2, false);
		...
		update : function(jq, _b7) {
			return jq.each(function() {
						...
						_b8.view.updateRow
								.call(_b8.view, this, _b7.id, _b7.row);
					});
		},
	...
	$.fn.treegrid.parseOptions = function(_b9) {
		return $.extend({}, $.fn.datagrid.parseOptions(_b9), $.parser
						.parseOptions(_b9, ["treeField", {
											animate : "boolean"
										}]));
	};
	var _ba = $.extend({}, $.fn.datagrid.defaults.view, {
		render : function(_bb, _bc, _bd) {
			...
			function _c3(_c4, _c5, _c6) {
				...
				var _c8 = (_c7 ? _c7.children.length : $(_bb)
				...
				for (var i = 0; i < _c6.length; i++) {
					...
					var _cd = _c0 + "-" + (_c4 ? 1 : 2) + "-"
							+ row[_be.idField];
					_c9.push("<tr id=\"" + _cd + "\" node-id=\""
							+ row[_be.idField] + "\" " + cls + " " + _cc + ">");
					_c9 = _c9.concat(_c1.renderRow.call(_c1, _bb, _bf, _c4,
							_c5, row));
					_c9.push("</tr>");
		...
		renderFooter : function(_ce, _cf, _d0) {
			...
			for (var i = 0; i < _d2.length; i++) {
				...
				_d4.push("<tr class=\"datagrid-row\" node-id=\""
						+ row[_d1.idField] + "\">");
			...
			for (var i = 0; i < _d6.length; i++) {
				...
				if (col) {
					...
					if (col.checkbox) {
						...
						cc.push(" name=\"" + _da + "\" value=\""
								+ (row[_da] != undefined ? row[_da] : "")
								+ "\">");
					} else {
						var val = null;
						if (col.formatter) {
		...
		deleteRow : function(_eb, id) {
			...
			if (_ed) {
				if (_ed.children.length == 0) {
					...
					tr.next("tr.treegrid-tr-tree").remove();
					var _ee = tr
							.children("td[field=\"" + _ec.treeField + "\"]")
							.children("div.datagrid-cell");
					_ee.find(".tree-icon").removeClass("tree-folder")
							.addClass("tree-file");
		...
		finder : {
			getTr : function(_109, id, type, _10a) {
				...
				if (_10a == 0) {
					...
					if (type == "body") {
						var tr = $("#" + $.data(_109, "datagrid").rowIdPrefix
								+ "-" + _10a + "-" + id);
						if (!tr.length) {
							tr = (_10a == 1 ? dc.body1 : dc.body2)
									.find("tr[node-id=\"" + id + "\"]");
						}
						return tr;
					} else {
						if (type == "footer") {
							return (_10a == 1 ? dc.footer1 : dc.footer2)
									.find("tr[node-id=\"" + id + "\"]");
						} else {
							if (type == "selected") {
								return (_10a == 1 ? dc.body1 : dc.body2)
