	...
	$.fn.resizable = function(options, param) {
		...
		function resize(e) {
			...
			if (resizeData.dir.indexOf('e') != -1) {
				...
				width = Math.min(Math.max(width, options.minWidth),
						options.maxWidth);
			...
			if (resizeData.dir.indexOf('s') != -1) {
				var height = resizeData.startHeight + e.pageY
						- resizeData.startY;
				height = Math.min(Math.max(height, options.minHeight),
						options.maxHeight);
			...
			if (resizeData.dir.indexOf('w') != -1) {
				...
				// resizeData.width = resizeData.startWidth - e.pageX +
				// resizeData.startX;
				// if (resizeData.width >= options.minWidth && resizeData.width
				// <= options.maxWidth) {
				// resizeData.left = resizeData.startLeft + e.pageX -
				// resizeData.startX;
				// }
			}
			if (resizeData.dir.indexOf('n') != -1) {
				...
				// resizeData.height = resizeData.startHeight - e.pageY +
				// resizeData.startY;
				// if (resizeData.height >= options.minHeight &&
				// resizeData.height <= options.maxHeight) {
				// resizeData.top = resizeData.startTop + e.pageY -
				// resizeData.startY;
				// }
			}
		}
		...
		return this.each(function() {
					...
					if (state) {
						...
						opts = $.extend({}, $.fn.resizable.defaults,
								$.fn.resizable.parseOptions(this), options
										|| {});
						$.data(this, 'resizable', {
									options : opts
								});
					...
					$(this).bind('mousemove.resizable', {
								...
								$(document).bind('mousedown.resizable', data,
										doDown);
								$(document).bind('mousemove.resizable', data,
										doMove);
								$(document).bind('mouseup.resizable', data,
										doUp);
