...
(function($) {
	...
	var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
		render : function(target, container, frozen) {
			...
			for (var i = 0; i < groups.length; i++) {
				table.push(this.renderGroup.call(this, target, i, groups[i],
						frozen));
			}
		...
		renderGroup : function(target, groupIndex, group, frozen) {
			...
			if (!frozen) {
				...
				table.push(opts.groupFormatter.call(target, group.value,
						group.rows));
				table.push('</span>');
			}
			table.push('</div>');
			...
			for (var j = 0; j < group.rows.length; j++) {
				...
				var cls = 'class="datagrid-row '
						+ (index % 2 && opts.striped
								? 'datagrid-row-alt '
								: ' ') + classValue + '"';
				var style = styleValue ? 'style="' + styleValue + '"' : '';
				var rowId = state.rowIdPrefix + '-' + (frozen ? 1 : 2) + '-'
						+ index;
				table.push('<tr id="' + rowId + '" datagrid-row-index="'
						+ index + '" ' + cls + ' ' + style + '>');
				table.push(this.renderRow.call(this, target, fields, frozen,
						index, group.rows[j]));
				table.push('</tr>');
				index++;
			}
		...
		bindEvents : function(target) {
			...
			body.unbind('click').bind('click', function(e) {
				...
				if (expander.length) {
					var gindex = expander.closest('div.datagrid-group')
							.attr('group-index');
	...
	$.extend($.fn.datagrid.methods, {
		...
		expandGroup : function(jq, groupIndex) {
			return jq.each(function() {
						...
						var group = view.find(groupIndex != undefined
	...
	$.extend(groupview, {
				refreshGroupTitle : function(target, groupIndex) {
					...
					var span = dc.body2
							.children('div.datagrid-group[group-index='
									+ groupIndex + ']')
							.find('span.datagrid-group-title');
					span.html(opts.groupFormatter.call(target, group.value,
							group.rows));
				},
				...
				insertRow : function(target, index, row) {
					...
					if (group) {
						...
						dc.body1.append(this.renderGroup.call(this, target,
								groupIndex, group, true));
						dc.body2.append(this.renderGroup.call(this, target,
								groupIndex, group, false));
					...
					function _moveTr(index, frozen) {
						...
						var prevTr = opts.finder.getTr(target, index - 1,
								'body', serno);
						var tr = opts.finder
								.getTr(target, index, 'body', serno);
				...
				updateRow : function(target, index, row) {
					...
					$.fn.datagrid.defaults.view.updateRow.call(this, target,
							index, row);
					var tb = opts.finder.getTr(target, index, 'body', 2)
				...
				deleteRow : function(target, index) {
					...
					$.fn.datagrid.defaults.view.deleteRow.call(this, target,
							index);
					...
					if (group.rows.length > 1) {
						...
						body.children('div.datagrid-group[group-index='
								+ groupIndex + ']').remove();
