...
(function($) {
	...
	function createBox(target) {
		...
		function setButtons(target) {
			...
			var button = $('<div class="datebox-button"><table cellspacing="0" cellpadding="0" style="width:100%"><tr></tr></table></div>')
					.appendTo(panel);
	...
	function setValue(target, value, remainText) {
		...
		if (remainText) {
			...
			if (value) {
				value = opts.formatter.call(target, calendar
								.calendar('options').current);
			}
	...
	$.fn.datebox.methods = {
		...
		initValue : function(jq, value) {
			return jq.each(function() {
						...
						if (value) {
							value = opts.formatter.call(this, opts.parser.call(
											this, value));
						}
						$(this).combo('initValue', value).combo('setText',
								value);
					});
		},
	...
	$.fn.datebox.parseOptions = function(target) {
		return $.extend({}, $.fn.combo.parseOptions(target), $.parser
						.parseOptions(target, ['sharedCalendar']));
	};
