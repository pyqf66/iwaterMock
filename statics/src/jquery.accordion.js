...
(function($) {
		...
		function _resize(collapsible, height) {
			...
			for (var i = 0; i < panels.length; i++) {
				...
				if (p.panel('options').collapsible == collapsible) {
					var pheight = isNaN(height)
							? undefined
							: (height + headerHeight * h.length);
					p.panel('resize', {
								width : cc.width(),
								height : (collapsible ? pheight : undefined)
							});
					totalHeight += p.panel('panel').outerHeight()
							- headerHeight * h.length;
				}
			}
			return totalHeight;
		}
	...
	/**
	 * find a panel by specified property, return the panel object or panel
	 * index.
	 */
	...
	function createPanel(container, pp, options) {
		...
		pp.panel($.extend({}, {
					...
					onBeforeExpand : function() {
						...
						if (!opts.multiple) {
							...
							var all = $.grep(getSelections(container),
									function(p) {
										return p.panel('options').collapsible;
									});
							for (var i = 0; i < all.length; i++) {
								unselect(container, getPanelIndex(container,
												all[i]));
							}
						}
					...
					onExpand : function() {
						...
						opts.onSelect.call(container,
								$(this).panel('options').title, getPanelIndex(
										container, this));
					},
					...
					onCollapse : function() {
						...
						opts.onUnselect.call(container, $(this)
										.panel('options').title, getPanelIndex(
										container, this));
					}
				}));
		...
		tool.children('a.panel-tool-collapse').hide(); // hide the old collapse
		// button
