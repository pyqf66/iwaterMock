...
(function($) {
	...
	function addTools(container) {
		...
		if (opts.tools) {
			if (typeof opts.tools == 'string') {
				...
				var tools = $('<div class="tabs-tool"><table cellspacing="0" cellpadding="0" style="height:100%"><tr></tr></table></div>')
						.appendTo(header);
	...
	function setSize(container, param) {
		...
		if (opts.tabPosition == 'left' || opts.tabPosition == 'right') {
			...
			header.add(panels)._size('height',
			...
			panels._size('height', isNaN(parseInt(opts.height)) ? '' : (cc
		...
		if (state.tabs.length) {
			...
			var hwidth = header.width()
					- header.children('.tabs-tool:not(.tabs-tool-hidden)')
							._outerWidth();
			var justifiedWidth = Math.floor((hwidth - d1 - d2
					* state.tabs.length)
					/ state.tabs.length);

			$.map(state.tabs, function(p) {
						setTabSize(p,
								(opts.justified && $.inArray(opts.tabPosition,
										['top', 'bottom']) >= 0)
										? justifiedWidth
										: undefined);
					});
			if (opts.justified
				...
				setTabSize(state.tabs[state.tabs.length - 1], justifiedWidth
								+ deltaWidth);
			}
		}
		...
		function setTabSize(p, width) {
			...
			var width = width ? width : (parseInt(p_opts.tabWidth
					|| opts.tabWidth || undefined));
			if (width) {
	...
	function bindEvents(container) {
		...
		$(container).children('div.tabs-header').unbind().bind('click',
			...
			if (li.length) {
				opts.onContextMenu
						.call(container, e, li.find('span.tabs-title').html(),
								getLiIndex(li));
			}
		});
	...
	function setProperties(container) {
		...
		header.removeClass('tabs-header-narrow').addClass(opts.narrow
				? 'tabs-header-narrow'
				: '');
	...
	function createTab(container, options, pp) {
		...
		pp.panel($.extend({}, options, {
					...
					onOpen : function() {
						...
						state.options.onSelect.call(container, popts.title,
								getTabIndex(container, this));
					},
					...
					onClose : function() {
						...
						var popts = $(this).panel('options');
						state.options.onUnselect.call(container, popts.title,
								getTabIndex(container, this));
					}
				}));
	...
	function addTab(container, options) {
		...
		if (options.selected) {
			selectTab(container, options.index); // select the added tab
			// panel
		}
	}
	...
	$.fn.tabs.methods = {
		...
		enableTab : function(jq, which) {
			return jq.each(function() {
						var opts = $(this).tabs('getTab', which)
								.panel('options');
						opts.tab.removeClass('tabs-disabled');
						opts.disabled = false;
					});
	...
	$.fn.tabs.parseOptions = function(target) {
		return $.extend({}, $.parser.parseOptions(target, ['tools',
