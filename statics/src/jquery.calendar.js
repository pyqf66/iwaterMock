...
(function($) {
	...
	function bindEvents(target) {
		...
		$(target).unbind('.calendar').bind('mouseover.calendar', function(e) {
			...
			if (t.hasClass('calendar-menu-next')
				...
				showYear(1);
			} else if (t.hasClass('calendar-menu-prev')
				...
				t.closest('div.calendar-body').find('.calendar-selected')
						.removeClass('calendar-selected');
				t.addClass('calendar-selected');
	...
	function show(target) {
		...
		var now = new Date();
		var todayInfo = now.getFullYear() + ',' + (now.getMonth() + 1) + ','
				+ now.getDate();
		var currentInfo = opts.current
	...
	$.fn.calendar = function(options, param) {
		...
		return this.each(function() {
			...
			bindEvents(this);
			show(this);
			$(this).find('div.calendar-menu').hide(); // hide the
				// calendar menu
			});
	};
		...
		moveTo : function(jq, date) {
			return jq.each(function() {
						...
						if (opts.validator.call(this, date)) {
							...
							if (!oldValue
									|| oldValue.getTime() != date.getTime()) {
								opts.onChange
										.call(this, opts.current, oldValue);
							}
						}
					});
		}
