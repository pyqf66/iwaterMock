...
(function($) {
	function init(target) {
		var slider = $('<div class="slider">' + '<div class="slider-inner">'
	...
	function setSize(target, param) {
		...
		if (opts.mode == 'h') {
			...
			slider
					...
					._outerHeight(slider._outerHeight());
		}
		initValue(target);
	}
	...
	function buildSlider(target) {
		...
		inner.html('<a href="javascript:void(0)" class="slider-handle"></a>'
		...
		slider.find('div.slider-inner').unbind('.slider').bind(
	...
	function setValues(target, values) {
		...
		for (var i = 0; i < values.length; i++) {
			...
			var input = $('<input type="hidden" class="slider-value">')
					.appendTo(slider);
			...
			if (opts.mode == 'h') {
				...
				tip.attr('style', style + 'margin-left:'
		...
		$(target).val(opts.range
				? newValues.join(opts.separator)
				: newValues[0]);

		if (oldValues.join(',') != newValues.join(',')) {
			opts.onChange.call(target, opts.value, (opts.range
							? oldValues
							: oldValues[0]));
		}
	}
