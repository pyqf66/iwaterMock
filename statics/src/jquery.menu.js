...
(function($) {
	...
	function init(target) {
		...
		opts.inline ? $(target).addClass('menu-inline') : $(target)
				.appendTo('body');
					...
					if (!item.hasClass('menu-sep')) {
						...
						if (itemOpts.iconCls) {
							$('<div class="menu-icon"></div>')
									.addClass(itemOpts.iconCls).appendTo(item);
						}
	...
	function setMenuSize(target, menu) {
		...
		menu.css('overflow', menu.outerHeight() < autoHeight
						? 'auto'
						: 'hidden');
		...
		menu.unbind('.menu').bind('mouseenter.menu', function() {
					...
					if (state.options.hideOnUnhover) {
						state.timer = setTimeout(function() {
									hideAll(target, $(target)
													.hasClass('menu-inline'));
	...
	$.fn.menu.defaults = {
		...
		minWidth : 120,
		itemHeight : 22,
		duration : 100, // Defines duration time in milliseconds to hide when
		// the mouse leaves the menu.
		hideOnUnhover : true, // Automatically hides the menu when mouse exits
		// it
