...
(function($) {
	function setSize(target, param) {
		...
		if (opts.width || opts.height || opts.fit) {
			...
			if (!isVisible) {
				var spacer = $('<div style="display:none"></div>')
						.insertBefore(target);
				var style = {
			...
			left.css('margin-top', parseInt((btn.height() - left.height()) / 2)
							+ 'px');
			if (!isVisible) {
	...
	function createButton(target) {
		...
		if (opts.iconCls) {
			$('<span class="l-btn-icon">&nbsp;</span>').addClass(opts.iconCls)
					.appendTo(inner);
	...
	function setDisabled(target, disabled) {
		...
		if (disabled) {
			...
			opts.plain
					? $(target).addClass('l-btn-disabled l-btn-plain-disabled')
