...
(function($) {
	...
	function loadData(target, data, remainText) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			if (g) {
				if (group != g) {
					...
					dd
							.push('<div id="'
									+ (state.groupIdPrefix + '_' + (state.groups.length - 1))
			...
			var cls = 'combobox-item'
					...
					+ (g ? ' combobox-gitem' : '');
			dd.push('<div id="' + (state.itemIdPrefix + '_' + i) + '" class="'
					+ cls + '">');
		...
		if (opts.multiple) {
			...
			setValues(target, selected.length
						...
						for (var i = 0; i < data.length; i++) {
							...
							if (opts.filter.call(target, q, row)) {
								...
								if (opts.groupField && group != g) {
									$('#' + state.groupIdPrefix + '_'
											+ $.inArray(g, state.groups))
											.show();
									group = g;
								}
							}
	...
	$.fn.combobox.parseOptions = function(target) {
		var t = $(target);
		return $
				.extend({}, $.fn.combo.parseOptions(target), $.parser
								.parseOptions(target, ['valueField',
	...
	$.fn.combobox.defaults = $.extend({}, $.fn.combo.defaults, {
		...
		finder : {
			...
			getRow : function(target, p) {
				var state = $.data(target, 'combobox');
				var index = (p instanceof jQuery) ? p.attr('id')
						.substr(state.itemIdPrefix.length + 1) : getRowIndex(
						target, p);
