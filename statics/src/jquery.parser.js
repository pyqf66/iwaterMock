...
(function($) {
				...
				for (var i = 0; i < properties.length; i++) {
					...
					if (typeof pp == 'string') {
						...
						for (var name in pp) {
							var type = pp[name];
							if (type == 'boolean') {
								opts[name] = t.attr(name)
										? (t.attr(name) == 'true')
										: undefined;
							} else if (type == 'number') {
								opts[name] = t.attr(name) == '0'
										? 0
	...
	$.fn._size = function(options, parent) {
		...
		function _setSize(target, property, parent, options) {
			...
			var min = $.parser.parseValue('min' + p1, options['min' + p1],
					parent);// || 0;
			var max = $.parser.parseValue('max' + p1, options['max' + p1],
					parent);// || 99999;
			var val = $.parser.parseValue(p, options[p], parent);
			var fluid = (String(options[p] || '').indexOf('%') >= 0
					? true
					: false);
