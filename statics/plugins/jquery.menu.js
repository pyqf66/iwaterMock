...
(function($) {
		...
		function _8(_c) {
			...
			if (_c.hasClass("menu-content")) {
				...
				_c.children("div").each(function() {
					var _d = $(this);
					var _e = $.extend({}, $.parser.parseOptions(this, ["name",
											"iconCls", "href", {
												separator : "boolean"
											}]), {
								disabled : (_d.attr("disabled")
										? true
										: undefined)
							});
					if (_e.separator) {
					...
					if (!_d.hasClass("menu-sep")) {
						...
						_d.empty().append($("<div class=\"menu-text\"></div>")
								.html(_f));
						if (_e.iconCls) {
							$("<div class=\"menu-icon\"></div>")
									.addClass(_e.iconCls).appendTo(_d);
						}
						if (_e.disabled) {
							_10(_3, _d[0], true);
						}
	...
	function _12(_14, _15) {
		...
		_15.removeClass("menu-noline")
				.addClass(_16.noline ? "menu-noline" : "");
	...
	function _28(_29, _2a) {
		...
		if (_2c.hasClass("menu-top")) {
			...
			if (_2b + _2c.outerWidth() > $(window)._outerWidth()
					+ $(document)._scrollLeft()) {
				_2b = $(window)._outerWidth() + $(document).scrollLeft()
						- _2c.outerWidth() - 5;
			}
			if (_2b < 0) {
				_2b = 0;
			...
			if (_2b + _2c.outerWidth() + 5 > $(window)._outerWidth()
					+ $(document).scrollLeft()) {
		...
		function _2e(top, _30) {
			if (top + _2c.outerHeight() > $(window)._outerHeight()
					+ $(document).scrollTop()) {
				if (_30) {
		...
		_2c.show(0, function() {
					if (!_2c[0].shadow) {
						_2c[0].shadow = $("<div class=\"menu-shadow\"></div>")
								.insertAfter(_2c);
					}
					_2c[0].shadow.css({
								display : (_2c.hasClass("menu-inline")
										? "none"
										: "block"),
	...
	function _3d(_3e, _3f) {
		...
		if (_3f.parent) {
			if (!_3f.parent.submenu) {
				var _42 = $("<div class=\"menu\"><div class=\"menu-line\"></div></div>")
						.appendTo("body");
				_42.hide();
		...
		if (_3f.iconCls) {
			$("<div class=\"menu-icon\"></div>").addClass(_3f.iconCls)
					.appendTo(_43);
		}
		if (_3f.id) {
			_43.attr("id", _3f.id);
					...
					if (_53) {
						...
						_53 = $.data(this, "menu", {
									options : $.extend({}, $.fn.menu.defaults,
											$.fn.menu.parseOptions(this), _51)
								});
						_2(this);
					}
					$(this).css({
								left : _53.options.left,
								top : _53.options.top
							});
				});
	};
	$.fn.menu.methods = {
		...
		setIcon : function(jq, _55) {
			return jq.each(function() {
						...
						if (_55.iconCls) {
							$("<div class=\"menu-icon\"></div>")
									.addClass(_55.iconCls).appendTo(_55.target);
						}
					});
		},
